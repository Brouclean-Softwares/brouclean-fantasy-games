...
use crate::data::blood_bowl::{coaches, games, players, teams};
...
pub async fn update(state: &AppState, coach: &User, game: Game) -> Result<(), AppError> {
    tracing::debug!("update by coach={:?} for game_id {}", coach, game.id,);
    ...
    .bind(game.id.clone())
    .bind(coach.id.unwrap_or(-1).clone())
    .bind(game.scheduled_at.clone())
